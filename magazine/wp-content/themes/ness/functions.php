...
if ( ! function_exists( 'cb_scripts_and_styles' ) ) {
    function cb_scripts_and_styles() {
      if ( !is_admin() ) {
        ...
        wp_register_style( 'cb-main-stylesheet',  get_template_directory_uri() . '/library/css/' . $cb_style_name . '.css', array(), '1.2', 'all' );
        wp_enqueue_style('cb-main-stylesheet'); // enqueue it
        // Register fonts
        $cb_font = cb_fonts();
        wp_register_style( 'cb-font-stylesheet',  $cb_font[0], array(), '1.2', 'all' );
        ...
        // ie-only stylesheet
        wp_register_style( 'cb-ie-only',  get_template_directory_uri(). '/library/css/ie.css', array(), '1.2' );
        ...
        if ( $cb_lightbox_onoff != 'off' ) {
            wp_register_script( 'cb-lightbox',  get_template_directory_uri() . '/library/js/jquery.fs.boxer.min.js', array( 'jquery' ), '1.2', true);
            wp_enqueue_script( 'cb-lightbox' ); // enqueue it
        }
        // Load Extra Needed scripts
        wp_register_script( 'cb-js-ext',  cb_file_location( 'library/js/cb-ext.js' ), array( 'jquery' ),'1.2', true);
        wp_enqueue_script( 'cb-js-ext' ); // enqueue it
        // Load scripts
        wp_register_script( 'cb-js',  cb_file_location( 'library/js/cb-scripts.js' ), array( 'jquery' ),'1.2', true);
...
if ( ! function_exists( 'cb_post_admin_scripts_and_styles' ) ) {
    function cb_post_admin_scripts_and_styles($hook) {
        ...
        if( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'profile.php' || $hook == 'appearance_page_ot-theme-options' || $hook == 'user-edit.php' || $hook == 'edit-tags.php' ) {
            ...
            wp_register_script( 'admin-js',  get_template_directory_uri() . '/library/admin/js/cb-admin.js', array(), '1.2', true);
